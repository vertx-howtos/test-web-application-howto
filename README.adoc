...
The pom contains JUnit 5 dependencies, https://vertx.io/docs/vertx-junit5/java/[vertx-junit5] and vertx-junit5-web-client[https://vertx.io/docs/vertx-junit5-web-client/java/] modules
...
To setup and tear down the test, deploy and undeploy the verticle like following:
[source,java,indent=0]
----
...
This assert that the HTTP response contains the correct status code, the correct `x-pet-id` header and correct body.
After the response is received, if the assertions are correct the test completes, otherwise it fails.


== Test get pet endpoint failures
...
When you pass a `Checkpoint` to `send*` methods of `TestRequest`, It doesn't complete the `VertxTestContext` but just flags the `Checkpoint`.
For more details on `VertxTestContext` and `Checkpoint`s, look at https://vertx.io/docs/vertx-junit5/java/#_checkpoint_when_there_are_multiple_success_conditions[Vertx JUnit 5 documentation].
`send*` methods return a `Future` that is completed after all assertions pass, so you can execute another request after the first one.
